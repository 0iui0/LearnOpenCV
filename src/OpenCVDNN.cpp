...
int main() {
    ...
    auto model = readNet("../model/frozen_inference_graph.pb",
                         "../model/ssd_mobilenet_v2_coco_2018_03_29.pbtxt", "TensorFlow");
    ...
    while (cap.isOpened()) {
        ...
        for (int i = 0; i < detectionMat.rows; i++) {
            ...
            if (confidence > 0.4) {
                ...
                putText(image, class_names[class_id - 1].c_str(), Point(box_x, box_y - 5), FONT_HERSHEY_SIMPLEX, 3, Scalar(0, 0, 255), 3);
            }
        }
